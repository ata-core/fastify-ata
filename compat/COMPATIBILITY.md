...
| Shared schema should be pass to serializer and validator ($ref to shared schema /definitions) | Genuine gap, on the roadmap: one shared-schema `$ref` shape still falls back to a path that cannot see external schemas. Tracked as an ata-core fix, not a plugin fix. |
...
So of the 8, six are AJV-identity tests and two are known items on the ata side, each tracked.

## Reading the number honestly

The suite contains 187 tests because it grew around AJV; a hypothetical perfect drop-in that is not AJV tops out at 181. ata is at 179 of those 181, with the last two named above. If you find a behavior difference not covered here, that is a bug in fastify-ata or ata: please open an issue.
